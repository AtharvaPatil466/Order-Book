...
struct BenchStats {
    ...
    void report() {
        ...
        std::cout << "Throughput:      " << std::fixed << std::setprecision(2) << (latencies.size() / (sum / 1e9)) / 1e6 << " M ops/sec" << std::endl;
        std::cout << "Average:         " << std::fixed << std::setprecision(1) << avg << " ns" << std::endl;
...
void runHighRigorBenchmark() {
    std::cout << "Starting Industrial-Rigor Benchmark (Individual Timing)..." << std::endl;
    OrderBook book;
    std::mt19937 rng(42);
    ...
    book.resetStatus();
    book.addOrder(nextId++, 999, Side::Buy, 1000000, 1, OrderType::Limit); // SET REF PRICE = 1,000,000
